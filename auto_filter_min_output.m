% 定义文件夹路径作为常量，命令行输出最小值对应波长，两位小数
folderPath = 'D:\OneDrive\桌面\实验室_微机光纤传感器\auto_filter_new\practice\';
% 定义保存结果的路径
outputFilePath = fullfile(folderPath, '20241120-2filtered.csv');
% 读取 CSV 文件 
filePath = fullfile(folderPath, '20241120-2.csv'); 
...
% 将极小值信息保存到单元格数组中
min_values{k} = table(min_wavelength, min_intensity);
end
% 将波长和滤波后的光强数据组合到一起
final_data = [wavelength, filtered_data];
% 将结果写入 CSV 文件
...
fprintf('组 %d 的极小值:\n', k);
% 检查极小值是否为空
if ~isempty(min_values{k})
min_wavelength = min_values{k}.min_wavelength;
min_intensity = min_values{k}.min_intensity;
% 确保两者长度一致
for i = 1:length(min_wavelength)
% 只输出两位小数
fprintf('波长: %.2f, 光强: %.2f\n', min_wavelength(i), min_intensity(i));
end
else
disp('没有找到极小值');
end
